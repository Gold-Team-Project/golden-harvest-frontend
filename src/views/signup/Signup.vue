      ...
      <div class="form-wrapper">
        ...
        <form @submit.prevent="handleSignup" class="signup-form">
          ...
          <div class="input-group">
            ...
            <input v-model="form.businessNumber" type="text" placeholder="사업자 번호를 입력하세요 (예: 000-00-00000)" />
          </div>
          ...
          <div class="input-group">
            ...
            <div class="file-upload-container">
              <div class="file-info-box">
                ...
                <input type="file" id="file-input-id" ref="fileInput" @change="handleFileUpload" hidden />
              </div>
...
// Swal 공통 스타일 설정 (borderRadius 제거하고 커스텀 스타일 사용 권장)
const commonSwal = {
  confirmButtonColor: '#11D411',
  cancelButtonColor: '#ef4444',
  customClass: {
    popup: 'rounded-popup' // 필요하다면 CSS에서 .rounded-popup { border-radius: 16px; } 정의
  }
};
...
const handleSendVerification = async () => {
  if (!form.email) {
    return Swal.fire({ ...commonSwal, title: '이메일 미입력', text: '이메일을 입력해주세요.', icon: 'warning' });
  }
  try {
    await authApi.sendEmail({ email: form.email, type: 'signup' });
    Swal.fire({ ...commonSwal, title: '발송 완료', text: '메일을 확인해주세요.', icon: 'success' });
  } catch (error) {
    Swal.fire({ ...commonSwal, title: '발송 실패', text: error.response?.data?.message || '실패했습니다.', icon: 'error' });
  }
};
...
const handleVerifyCode = async () => {
  try {
    ...
    Swal.fire({ ...commonSwal, title: '인증 성공', text: '완료되었습니다.', icon: 'success' });
  } catch (error) {
    Swal.fire({ ...commonSwal, title: '인증 실패', text: '번호가 틀렸습니다.', icon: 'error' });
  }
};

const handleSignup = async () => {
  if (form.password !== form.passwordConfirm) {
    return Swal.fire({ ...commonSwal, title: '비밀번호 불일치', icon: 'error' });
  }
  if (!selectedFile.value) {
    return Swal.fire({ ...commonSwal, title: '서류 미첨부', icon: 'warning' });
  }

  try {
    // 403 에러 방지를 위해 데이터 구성 확인 필요
    const signUpData = {
      email: form.email,
    ...
    // authApi.signup 내부에서 FormData를 사용하여 파일을 함께 보내는지 확인하세요.
    const res = await authApi.signup(signUpData, selectedFile.value);

    await Swal.fire({ ...commonSwal, title: '신청 완료', icon: 'success' });
    location.href = '/login';
  } catch (error) {
    console.error(error);
    Swal.fire({
      ...commonSwal,
      title: '가입 실패 (403)',
      text: error.response?.status === 403 ? '접근 권한이 없거나 서버 설정(CSRF/CORS)을 확인하세요.' : '에러가 발생했습니다.',
      icon: 'error'
    });
  }
};
...
.file-info-box { flex: 1; display: flex; align-items: center; background: white; border: 1px solid #C8E4C8; border-radius: 10px; padding: 0 15px; height: 48px; box-sizing: border-box; overflow: hidden; }
.file-select-label { background: #E2E8F0; padding: 6px 12px; border-radius: 6px; font-size: 12px; cursor: pointer; color: #444; font-weight: 700; margin-right: 12px; white-space: nowrap; }
.file-name-text { font-size: 13px; color: #666; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
...
.login-link { color: #11D411; font-weight: 700; text-decoration: none; }
